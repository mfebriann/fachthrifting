...
<body>
  ...
  if (isset ($_POST['submit'])) {
    ...
    $adminFee = str_replace(',', '', htmlspecialchars(str_replace('.', '', $_POST['biaya-admin']), ENT_QUOTES, 'UTF-8'));
    $description = htmlspecialchars($_POST['keterangan'], ENT_QUOTES, 'UTF-8');

    if ($isProduct === 'yes') {
      $size = htmlspecialchars($_POST['ukuran'], ENT_QUOTES, 'UTF-8');
      $stok = htmlspecialchars($_POST['stok'], ENT_QUOTES, 'UTF-8');
      ...
      $hargaItem = str_replace(',', '', htmlspecialchars(str_replace('.', '', $_POST['harga-item']), ENT_QUOTES, 'UTF-8'));
      $hargaMaxNego = str_replace(',', '', htmlspecialchars(str_replace('.', '', $_POST['harga-max-nego']), ENT_QUOTES, 'UTF-8'));
      $resi = htmlspecialchars($_POST['resi-kurir'], ENT_QUOTES, 'UTF-8');

      mysqli_query($conn, "UPDATE `transactions` SET `message` = '$pesanTransaksi', `isproduct` = '$isProduct', `status` = '$status', `size` = '$size', `stok` = $stok, `category` = '$category', `total_price` = '$totalHarga', `price_item` = '$hargaItem', `price_max_nego` = '$hargaMaxNego', `admin_fee` = '$adminFee', `resi` = '$resi', `description` = '$description', `dateupdated` = NOW() WHERE transaction = $id");
    } else {
      mysqli_query($conn, "UPDATE `transactions` SET `message` = '$pesanTransaksi', `isproduct` = '$isProduct', `status` = '$status', `total_price` = '$totalHarga', `admin_fee` = '$adminFee', `description` = '$description', `dateupdated` = NOW() WHERE transaction = $id");
    }
