...
<body>
  ...
  if (isset ($_GET['date'])) {
    $date = $_GET['date'];
    $query = mysqli_query($conn, "SELECT `transaction`, `message`, `author`, `status`, `stok`, `category`, `resi`, `total_price`, `admin_fee`,  `datetime` FROM `transactions` WHERE DATE_FORMAT(`datetime`, '%Y-%m') = '$date' ORDER BY transaction DESC");
  } else {
